...
Assets use the **esbuild + Tailwind v4 standalone binaries** (no Node.js / `package.json`). The Tailwind theme tokens (`--color-paper/ink/accent`, fonts) live in `assets/css/app.css`; `hero-*` icon utilities are generated by `assets/vendor/heroicons.js` from the `heroicons` dep.
...
- **Components:** `CoreComponents` (button/input/icon/flash, `@spec`'d, Gettext-backed), `Layouts` (root + app), `ErrorHTML`. UI follows wireframe variant **1a** (stacked), copy is intentionally Italian.
